...
namespace Wings.Core
{
    ...
    /// 表示应用层服务的抽象类。
    /// </summary>
    public abstract class CoreService : DisposableObject
    {
        ...
        public CoreService(IRepositoryContext context)
        {
        ...
        {
            Mapper.CreateMap<BaseDTO, AggregateRoot>().ForMember(root => root.ID, dto => dto.ResolveUsing(d =>
            {
                Guid result = Guid.Empty;
                Guid.TryParse(d.ID, out result);
                return result;
            }));
            Mapper.CreateMap<AggregateRoot, BaseDTO>().ForMember(dto => dto.ID, root => root.ResolveUsing(d =>
            {
            ...
            Mapper.CreateMap<RoleDTO, Role>().ForMember(root => root.ID, dto => dto.ResolveUsing(d =>
            {
                Guid result = Guid.Empty;
                Guid.TryParse(d.ID, out result);
                return result;
