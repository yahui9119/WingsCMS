...
namespace Wings.Framework.Utils.ValidateCode
{
    public  class VerificationCode
    {
        ...
        private void IniSession(string verifyCode)
        {
            System.Web.HttpContext.Current.Session["verifyCode"] = verifyCode.ToLower();
        }
        public MemoryStream Create(string verifyCode)
        {
            IniSession(verifyCode);
            this.VarifyCode = verifyCode;
        ...
        //        var s = this.Create(verifyCode, m);
        //        bytesstr= Encoding.Default.GetString(m.ToArray());
        //    }
        //    return Encoding.Default.GetBytes(bytesstr);
        //}
        public static bool TestCode(string text)
        {
            var verifyCode = System.Web.HttpContext.Current.Session["verifyCode"] ;
            if (verifyCode != null)
            {
                return text.ToLower() == verifyCode.ToString().ToLower();
            }
            return false;
        }
